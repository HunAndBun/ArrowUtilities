...
namespace ArrowUtilities.TilemapTools {
    ...
    public static class TileHelperFunctions {
        ...
        /// <summary>
        ///     <para>
        ///         Helper method for setting a square of tiles all at once.
        ///         <br />
        ///         <b>
        ///             Note: This statically caches the values for size and tile, so continuous usage (with the same parameters)
        ///             remains performant.
        ///         </b>
        ///     </para>
        /// </summary>
        ...
        public static void SetSquareOfTiles(this Tilemap tilemap, Vector3Int bottomLeftCorner, int size, Tile tile) {
			...
			if (size == _size) //If the size chosen is the same as the previous usage...
			{
				...
				{
					tilemap.SetTilesBlock(boundsInt,
						_tilesForSquare); //Reuse both (max performance) and do the final set.
					return;
				}
			}
        ...
        /// <summary>
        ///     <para>
        ///         Helper method for setting a rectangle of tiles all at once.
        ///         <br />
        ///         <b>
        ///             Note: This statically caches the values for size and tile, so continuous usage (with the same parameters)
        ///             remains performant.
        ...
        public static void SetRectangleOfTiles(this Tilemap tilemap, Vector3Int bottomLeftCorner, int height, int width,
			...
			if (height == _height && width == _width) //If the size chosen is the same as the previous usage...
			{
				...
				{
					tilemap.SetTilesBlock(boundsInt,
						_tilesForRectangle); //Reuse both (max performance) and do the final set.
					return;
				}
			}
			else {
			...
			_tilesForRectangle =
				EnumerableHelpers.GetPreFilledArray<TileBase> //Get array filled with the same tile, and cache it too.
				(
					boundsInt.size.x * boundsInt.size.y *
					boundsInt.size
						.z, //Tilemaps are technically 3D (sorta) so you need the same number of tiles as a "rectangular prism" of them would entail.
					tile
				);
